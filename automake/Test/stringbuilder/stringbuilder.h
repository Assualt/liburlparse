...
public: //constructor
    ...
    StringBuilderImpl & operator = (StringBuilderImpl & builder)=delete;
    StringBuilderImpl(const string_t &src):m_nprecision(15) {
        if (!src.empty()) {
            m_Data.push_back(src);
        }
        m_totalSize = src.size();
    }
    StringBuilderImpl():m_totalSize(0),m_nprecision(15) {
    }
...
public: // insert delete remove clear append
    ...
    StringBuilderImpl& setSize(size_type size) {
        return setLength(size);
    }
    StringBuilderImpl &setLength(size_type len) {
        if(len == 0){// remove all
            clear();
            return *this;
        }
    ...
    StringBuilderImpl &remove(int start, int end) {
    ...
    StringBuilderImpl &removeCharAt(int index) {
        if (static_cast<size_type>(index) > m_totalSize) {
            return *this;
        } else {
    ...
    template<class ArgType>
    StringBuilderImpl &append(const ArgType & src){
        Append(toString(src));
        return *this;
    }
    StringBuilderImpl &insert(int ops, chr c) {
    ...
    StringBuilder & replace(int start, int end, string_t str)
    {
            Delete1(start, end);
            int i = 0;
            for (;i < (int) str.size();)
    ...
    */
    StringBuilderImpl &replace(int start, int end,const string_t &str) {
        remove(start, end);
