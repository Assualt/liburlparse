...
public:
    ...
    string_t substr(off_t nBegin, off_t nEnd = -1) {
        ...
        return string_t(m_Data + nBegin, nEnd - nBegin);
    }
    ...
    StringBuilderImpl &setSize(off_t nSize) {
        if (nSize > m_nTotalSize) {
            ...
            (void)memset(m_Data + nSize, 0, m_nTotalSize - nSize);
            m_nTotalSize = nSize;
        }
        return *this;
    }
    ...
    StringBuilderImpl &remove(off_t nStart, off_t nEnd = -1) {
        ...
        // memset [nStart,nEnd)
        (void)memset(m_Data + nStart, 0, nEnd - nStart);
        // copythe [nEnd, npos) to => [nStart, nEnd)
        (void)memcpy(m_Data + nStart, m_Data + nEnd, m_nTotalSize - nEnd);
        // clear the (nEnd, nPos)
        (void)memset(m_Data + nStart + m_nTotalSize - nEnd, 0, nEnd - nStart);
    ...
    StringBuilderImpl &insert(off_t nIndex, chr arg, size_type nCnt = 1) {
        ...
        (void)memcpy(m_Data + nIndex + nCnt, m_Data + nIndex, m_nTotalSize - nIndex);
        (void)memset(m_Data + nIndex, 0, nCnt);
        (void)memcpy(m_Data + nIndex, temp.c_str(), nCnt);
        m_nTotalSize += nCnt;
        return *this;
    }
    ...
    StringBuilderImpl &replace(off_t nStart, off_t nEnd, const string_t &str) {
        ...
        (void)memcpy(m_Data + nStart + str.size(), m_Data + nEnd, m_nTotalSize - nEnd);
        // move str to (nStart, nStart+str.size()]
        (void)memcpy(m_Data + nStart, str.c_str(), str.size());
        return *this;
    }
