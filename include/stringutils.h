...
public:
	// static void split(list<string> &splitList, const string &s, const string &seperator);
	static void split(std::list<std::string>& v,const std::string& s,  const std::string& c);
	//static bool trans(const char *pFromCode, const char *pToCode, const char *pInBuf, size_t iInLen, char *pOutBuf, size_t iOutLen );
