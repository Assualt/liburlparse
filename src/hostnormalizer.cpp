...
std::vector<std::string> HostNormalizer::splitbyDot(const std::string &src, const char c, int nlimit) {
    ...
    if (!temp.empty())
        ret.push_back(temp);
    for (size_t i = ret.size(); i < nCnt + 1; ++i) ret.push_back("");
    if (nlimit < 0) {
        return ret;
    } else if (nlimit == 0) {
    ...
    if (size < nlimit)
        return ret;
    for (int i = 0; i < nlimit; ++i) rcpy.push_back(ret[i]);
    return rcpy;
}
...
void HostNormalizer::normalizeHost() {
    ...
    try {
        ...
        for (size_t i = 0; i < _host.size(); ++i) { host.push_back(_host[i] & 0xFF); }
    } catch (...) {
        // occurs when the url is invalid. Just return
        Log().log()
                .setLevel(LOG_DEBUG_LEVEL)
                .format("IllegalArgumentException happend. [at FILE:%s FUNC:%s "
                        "LINE:%d]",
                        __FILE__,
                        __FUNCTION__,
                        __LINE__)
    ...
    if (!_bytes.empty()) {
        try {
            ...
            Log().log()
                    ...
                    .format("IPADDRESS Can't convert to host(string handler error). [at FILE:%s FUNC:%s LINE:%d]",
                            __FILE__,
                            __FUNCTION__,
                            __LINE__)
                    .toFile();
...
vector<ubyte>
        &HostNormalizer::tryDecodeHostToIPv4(std::vector<ubyte> &bytes, const std::string &host, bool &validEmpty) {
