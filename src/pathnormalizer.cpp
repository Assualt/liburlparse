...
std::string PathNormalizer::sanitizeDotsAndSlashes(string &path)
{
	...
	while (index < (int)stringBuilder.size() - 1)
	{
		if (stringBuilder.charAt(index) == '/')
		{
			...
			{
				if (index < (int)stringBuilder.size() - 2 && stringBuilder.charAt(index + 2) == '.')
				{
					//If it looks like "/../" or ends with "/.."
					if (((index < (int)stringBuilder.size() - 3) && (stringBuilder.charAt(index + 3) == '/')) ||
						(index == (int)stringBuilder.size() - 3))
					{
						bool endOfPath = index == ((int)stringBuilder.size() - 3);
						slashIndexStack.pop();
						int endIndex = index + 3;
						// backtrack so we can detect if this / is part of another replacement
						
						index = slashIndexStack.empty() ? -1 : slashIndexStack.top() - 1;
						slashIndexStack.pop();
						int startIndex = endOfPath ? index + 1 : index;
						stringBuilder.Delete(startIndex + 1, endIndex);
					}
				}
				else if (((index < (int)stringBuilder.size() - 2) && (stringBuilder.charAt(index + 2) == '/'))
