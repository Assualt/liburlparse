...
std::set<std::string> UrlDetector::VALID_SCHEMES =
        {"http://", "https://", "ftp://", "ftps://", "http%3a//", "https%3a//", "ftp%3a//"};
...
const std::list<Url> &UrlDetector::detect() {
    ...
    Log().log()
            ...
            .format("URL Detector begin. [at FILE:%s FUNC:%s LINE:%d]", __FILE__, __FUNCTION__, __LINE__)
            .toFile();
    start = clock();
    readDefault();
    end = clock();
    Log().log()
            .setLevel(LOG_DEBUG_LEVEL)
            .format("URL Detector end.Cost time:%f s. [at FILE:%s FUNC:%s LINE:%d]",
...
int UrlDetector::processColon(int length) {
    if (_hasScheme) {
        ...
        length = _buffer.size();  // set length to be right after the scheme
            ...
            _buffer.size() > 0 && _options.hasFlag(ALLOW_SINGLE_LEVEL_DOMAIN) &&
...
CharacterMatch UrlDetector::checkMatchingCharacter(char curr) {
    ...
    if ((curr == '\"' && _options.hasFlag(QUOTE_MATCH)) || (curr == '\'' && _options.hasFlag(SINGLE_QUOTE_MATCH))) {
        ...
        return CharacterMatchStart;
            ...
            (_options.hasFlag(BRACKET_MATCH) && (curr == ']' || curr == '}' || curr == ')')) ||
    ...
    while (!done && !_reader.eof()) {
        ...
        if (curr == '@') {
            ...
            rollback = true;
                ...
                curr == '#' || curr == ' ' || curr == '/' || checkMatchingCharacter(curr) != CharacterNotMatched ||
                curr == ',' || curr == ';') {
